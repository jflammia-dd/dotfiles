...
- No comma before a coordinating conjunction ("and," "but," "or"). Drop the comma and let the conjunction do the work. "The math is straightforward and the team agrees" not "The math is straightforward, and the team agrees." If the sentence feels too long without the pause, split it into two sentences instead.
  - **High-risk pattern: compound sentences with a natural pause.** Any sentence joined by "and", "but" or "or" tends to attract a comma before the conjunction because the breath feels natural. It isn't. "ERS queries the state store, and compares the result" is wrong. "ERS queries the state store and compares the result" is correct. If removing the comma makes the sentence feel rushed, split it into two sentences. Never insert a comma to manufacture a pause.
...
- "Almost" as a correction opener. It evaluates the person negatively before you've said anything useful. Start with the correct framing directly. "Almost. The retention is actually controlled at the scope level" is wrong. "Retention is controlled at the scope level" is right.
- Surfacing your reasoning context in the reply. If you know a colleague was part of a prior conversation that informs how you're framing the answer, that knowledge helps you draft but doesn't belong in the reply. "Since you've been in that conversation" is your reasoning, not their cue.
- Citing specific numbers you can't verify. If a figure comes from a derived calculation, an AI-generated summary or a secondhand source, use qualitative framing ("orders of magnitude smaller") rather than a specific number you can't stand behind. A wrong number in a Confluence reply undermines trust more than a general statement.
- Deferring operational concerns entirely to a PoC. When a reviewer flags something worth watching over time, the right response commits to ongoing tracking as the system scales, not "we'll benchmark this in the PoC." The PoC is one data point; the concern is about production behavior across many orgs and customers.
- Curt technical statements in collegial contexts. "The pattern is consistent with every other direct-write producer in the codebase" is technically correct but reads as dismissive. The same information lands better when it invites the reader in: "Every other direct-write producer in the codebase takes the same approach. The appendix covers all five if you want to see the pattern in context."
